...
CONTROL supports **one local HTTP security model**: loopback binding plus strict Host, Origin and browser request-metadata checks, with the installed Collector extension ID explicitly allowlisted. There is no supported HTTP bearer-token mode. The retired `SHINO_CONTROL_TOKEN` environment setting is ignored if left over from an earlier installation; the Dashboard, Collector, Supervisor and Tray do not require a CONTROL token. The Collector only connects to HTTP loopback endpoints (`127.0.0.1` or `localhost`), including an optional custom local port. None of these protections prevents a malicious process already running on the same computer from accessing loopback; run CONTROL only on a trusted machine. The separate `GITHUB_TOKEN` / fine-grained GitHub token is still available exclusively for repository sync.
