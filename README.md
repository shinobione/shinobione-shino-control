...
CONTROL is a **loopback-only** application; a non-loopback deployment is not supported. All HTTP routes now reject unexpected Host values and foreign browser origins. Local dashboard requests use same-origin access, and cross-origin CORS responses are disabled.
