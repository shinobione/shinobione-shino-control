...
const ACTION_SIGNAL = /(next|resume|retest|needs? test|pending|physical gate|live test|todo|remaining|needs? to|should|must|verify|validate|\btest\b|run|open|implement|build|fix|update|send|reply|review|continue|try|compare|choose|install|configure|check|tell|share|give|upload)/i;
