...
function classifyChat(e) {
  ...
  if (usefulResume(resume)) {
    if (BLOCKED.test(resume)) return { status:'BLOCKED', source:'chat_resume_action', reason:'The current ChatGPT resume action is blocked by an unresolved dependency.' };
  ...
  if (BLOCKED.test(stateText)) return { status:'BLOCKED', source:'chat_state_summary', reason:'The current ChatGPT state summary contains an unresolved blocker.' };
...
function classifyGithubLike(e) {
  ...
  if (hint) return hint;
  if (BLOCKED_GITHUB.test(full)) return { status:'BLOCKED', source:'source_text', reason:'Source text contains an unresolved blocker.' };
