...
import { CHATGPT_STATE_SCHEMA_VERSION } from './chatgpt-tail-state.mjs';
...
function keyFromUrl(value = '') {
  try { return new URL(value).pathname.match(/\/c\/([^/?#]+)/i)?.[1] || null; }
  catch { return null; }
}
...
function ownershipMismatch(state, source, thread) {
  ...
  if (exact && exact.id !== source.projectId) {
    return { expectedProjectId:exact.id, currentProjectId:source.projectId || null, reason:'project-ownership-mismatch' };
  }
  ...
  if (!exact && mappedId && mappedId !== source.projectId) {
    return { expectedProjectId:mappedId, currentProjectId:source.projectId || null, reason:'project-key-mismatch' };
  }
  return null;
}

function timeoutQuarantineApplies(entry, remoteAt, nowMs) {
  if (!entry) return false;
  const retryAt = timeMs(entry.retryAt);
  if (!retryAt || retryAt <= nowMs) return false;
  const markedRemoteAt = timeMs(entry.remoteUpdatedAt);
  // A newly updated remote conversation must bypass quarantine immediately.
  if (remoteAt && markedRemoteAt && remoteAt !== markedRemoteAt) return false;
  if (remoteAt && !markedRemoteAt) return false;
  return true;
}
...
export function planChatgptCatchup(state, inventory = {}, options = {}) {
  ...
  const inaccessibleByKey = state.settings?.chatgptInaccessible || {};
  const timeoutQuarantineByKey = state.settings?.chatgptTimeoutQuarantine || {};
  const changed = [];
  let unchanged = 0;
  ...
  let stateSchemaUpgrades = 0;
  let inaccessibleCount = 0;
  let timeoutQuarantinedCount = 0;
  ...
  for (const thread of byKey.values()) {
    ...
    const inaccessible = inaccessibleByKey[thread.key];
    const inaccessibleRemoteAt = timeMs(inaccessible?.remoteUpdatedAt);

    if (inaccessible && ((!remoteAt && !inaccessibleRemoteAt) || (remoteAt && remoteAt === inaccessibleRemoteAt))) {
      inaccessibleCount++;
      continue;
    }

    const timeoutQuarantine = timeoutQuarantineByKey[thread.key];
    if (timeoutQuarantineApplies(timeoutQuarantine, remoteAt, nowMs)) {
      timeoutQuarantinedCount++;
      continue;
    }

    const timeoutRetryMeta = timeoutQuarantine ? {
      slowRetry:true,
      timeoutAttempts:Number(timeoutQuarantine.attempts || 0)
    } : {};

    if (!source) {
      newlyDiscovered++;
      changed.push({...thread, ...timeoutRetryMeta, reason:'new-conversation', localUpdatedAt:null});
      continue;
    }
    ...
    if (mismatch) {
      ownershipMismatches++;
      changed.push({...thread, ...timeoutRetryMeta, ...mismatch, localUpdatedAt:localAt ? new Date(localAt).toISOString() : null});
      continue;
    }

    if (Number(source.chatgptStateSchemaVersion || 0) !== CHATGPT_STATE_SCHEMA_VERSION) {
      stateSchemaUpgrades++;
      changed.push({
        ...thread,
        ...timeoutRetryMeta,
        reason:'state-schema-upgrade',
    ...
    if (!localAt) {
      baselineMissing++;
      changed.push({...thread, ...timeoutRetryMeta, reason:'baseline-missing', localUpdatedAt:null});
      continue;
    }

    if (remoteAt > localAt + toleranceMs) {
      changed.push({...thread, ...timeoutRetryMeta, reason:'remote-newer', localUpdatedAt:new Date(localAt).toISOString()});
    } else {
      unchanged++;
    }
  }
