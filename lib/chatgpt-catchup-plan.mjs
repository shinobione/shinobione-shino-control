...
function latestLocalTimestamp(state, source, key) {
  ...
  for (const evidence of state.evidence || []) {
    ...
    // Only compare source-native update timestamps. collectorObservedAt / lastObservedAt and a
    // generic evidence timestamp can be later than the actual conversation edit time and would
    // incorrectly hide a stale local capture.
