...
export function planChatgptCatchup(state, inventory = {}, options = {}) {
  ...
  for (const thread of byKey.values()) {
    ...
    if (mismatch) {
      ownershipMismatches++;
      changed.push({...thread, ...mismatch, localUpdatedAt:localAt ? new Date(localAt).toISOString() : null});
      continue;
    }
    ...
    if (remoteAt > localAt + toleranceMs) {
      changed.push({...thread, reason:'remote-newer', localUpdatedAt:new Date(localAt).toISOString()});
    } else {
      unchanged++;
    }
  }
