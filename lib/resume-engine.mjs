...
const NO_ACTION = /\b(no remaining work|nothing left|no further action|no action required|nothing to do|fully complete|closeout complete|finished)\b/i;
...
function actionScore(text = '') {
  if (NO_ACTION.test(text)) return -100;
  let score = 0;
...
export function deriveResume(project, evidence, fallback = 'Review the newest evidence and choose the next safe action.') {
  ...
  if (evidence.sourceType !== 'chatgpt_thread') {
    if (isUseful(evidence.resumeAction) && !NO_ACTION.test(cleanText(evidence.resumeAction))) {
...
export function resumeTextIsUseful(value = '') {
  return isUseful(value) && !NO_ACTION.test(cleanText(value));
}
