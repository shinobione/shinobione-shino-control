...
function signalLines(messages = []) {
  const lines = transcriptOf(messages)
    .split(/\n+/)
    .map(cleanText)
    .filter(Boolean);
  return lines.filter(line => /(pass|fail|merged|blocked|blocker|next|pending|test|validated|deployed|supersed|do not merge|resume|physical|synced|fixed|works|working|shipped|todo|remaining|complete|done|wait|verify|check|install|configure|build|implement|review|send|reply)/i.test(line));
}

function summarize(messages = []) {
  const signals = signalLines(messages);
  const transcript = transcriptOf(messages);
  const summary = signals.slice(-5).join(' · ').slice(0, 1800) || transcript.slice(-1800) || 'ChatGPT delta collected.';
  const next = [...signals].reverse().find(line => /(next|resume|retest|needs? test|pending|physical gate|live test|todo|remaining|needs? to|should|must|verify|validate|run|open|implement|build|fix|update|send|reply|review)/i.test(line));
  return {
    summary,
    currentStateSummary:summary.slice(0, 700),
    resumeAction:(next || '').replace(/^\s*(?:ASSISTANT|USER|NEXT|TODO|ACTION)\s*[:—-]\s*/i, '').slice(0, 700) || 'Continue in the synced ChatGPT thread.'
  };
}
...
function upsertDiscovered(state, payload, key, messages) {
  ...
  const row = {
    ...
    preview:transcriptOf(messages).slice(-500),
