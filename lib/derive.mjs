...
export function deriveProjectState(project, evidence, context = {}) {
  ...
  if (!usable.length) {
    ...
    // project with zero conversations. No inference from missing thread sources is allowed.
...
export function deriveAll(state) {
  ...
  const inventoryProjects = Array.isArray(state.settings.lastChatgptInventory?.projects)
    ? state.settings.lastChatgptInventory.projects
    : [];
