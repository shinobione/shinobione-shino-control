...
function statusText(e) {
  if (!e) return '';
  // Chat transcripts are intentionally not treated as project-state documents. Their summaries
  // may contain many historical words such as "test", "blocked", "pass" and "fixed" from the
  // whole discussion. For chat-only evidence, only the title is allowed to emit a state signal.
...
function statusSignal(e) {
  ...
  // Old ChatGPT ingestion generated derivedStatusHint from transcript keyword matches. Those hints
  // are too noisy to drive project state. Hard hints remain authoritative for non-chat sources.
...
export function deriveProjectState(project, evidence) {
  ...
  // Newest explicit state wins. Because chat transcript bodies are excluded from state signalling,
  // ordinary project chatter no longer creates false NEEDS TEST/BLOCKED cards.
