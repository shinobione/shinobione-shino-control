...
const MANUAL_STATUS_VALUES = new Set(['ACTIVE','NEEDS TEST','BLOCKED','STABLE','WAITING','DONE','EMPTY','UNSYNCED']);
...
function projectDerivationFingerprint(project, projectEvidence, context) {
  const evidence = [...projectEvidence]
    .filter(e => e.inventoryCurrent !== false && e.controlExcluded !== true)
  ...
  const payload = {
    ...
    project: {
      ...
      statusOverride: MANUAL_STATUS_VALUES.has(String(project.control?.statusOverride || '').toUpperCase()) ? String(project.control.statusOverride).toUpperCase() : null
    },
    context: {
...
export function deriveProjectState(project, evidence, context = {}) {
  const all = [...evidence]
    .filter(e => e.projectId === project.id && e.inventoryCurrent !== false && e.controlExcluded !== true)
...
function applyProjectControl(project, derived) {
  const override = String(project?.control?.statusOverride || '').toUpperCase();
  if (!MANUAL_STATUS_VALUES.has(override)) return derived;
  return {
    ...derived,
    autoStatus:derived.status,
    status:override,
    statusSource:'manual_override',
    statusReason:'Project state manually overridden in SHINO // CONTROL.',
...
export function deriveAll(state, options = {}) {
  ...
  state.derived = visibleProjects.map(project => {
    ...
    return {
      ...applyProjectControl(project, deriveProjectState(project, projectEvidence, context)),
      derivedAt: now
    };
  });
  ...
  if (legacyConservationBase && !legacyEmptyProjectId) {
    ...
    if (conservationProvesSoleEmpty) {
      ...
      if (project && index >= 0) {
        ...
        state.derived[index] = {
          ...applyProjectControl(project, deriveProjectState(project, projectEvidence, proofContext)),
          derivedAt: now
        };
