...
export function deriveProjectState(project, evidence, context = {}) {
  ...
  // Explicit project-state hints are curated/structured evidence. Prefer the newest such hint over
  // generic words found in newer commits or summaries. This keeps an unresolved physical gate from
  // being accidentally turned STABLE by an unrelated later commit containing words like complete.
...
export function deriveAll(state) {
  ...
  // A CONTROL state may contain more current source rows than the authoritative API conversation
  // total because older ingestion runs can leave two source records for the same conversation UUID.
  // Counts used for project state must therefore be conversation-unique, never raw source-row counts.
  ...
  // Legacy v0.7.2 conservation fallback. Old CONTROL states saved the authoritative aggregate
  // project/conversation totals but not the per-project rows. Stale remembered mapping keys can
  // outlive projects, and duplicate current source rows can outlive old ingestion passes. The only
  // trustworthy quantities are unique current conversation UUIDs and distinct current project keys.
  // If those unique conversations account for the API total and span exactly projectCount - 1 keys,
  // then exactly one API project is empty. If CONTROL has exactly one UNSYNCED project with no current
  // conversation, that project is the empty one.
  if (inventoryProjects.length === 0) {
    ...
    const representedConversationCount = currentConversationKeys.size;
    ...
    if (conservationProvesSoleEmpty) {
      ...
      if (project && index >= 0) {
        state.derived[index] = deriveProjectState(project, state.evidence, {
          ...
          currentChatCount: 0,
          inventoryProof: 'legacy-conservation-unique-conversations'
        });
      }
    }
  }
