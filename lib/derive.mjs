...
function statusText(e) {
  if (!e) return '';
  // Chat transcripts are evidence, not project-state documents. Their bodies can contain weeks of
  // historical words such as test/blocked/pass. Only a chat title may emit a state signal.
...
function statusSignal(e) {
  ...
  if (e.sourceType !== 'chatgpt_thread') {
...
export function deriveProjectState(project, evidence, context = {}) {
  ...
  if (!usable.length) {
    // EMPTY is only asserted when the latest ChatGPT API inventory explicitly contains this
    // project with zero conversations, or when the legacy aggregate inventory mathematically
    // proves the sole missing project has zero conversations (see deriveAll conservation fallback).
...
export function deriveAll(state) {
  ...
  const inventoryProjects = Array.isArray(inventorySummary.projects)
    ? inventorySummary.projects
    : [];
  ...
  for (const source of state.sources) {
    ...
    currentChatCounts.set(source.projectId, (currentChatCounts.get(source.projectId) || 0) + 1);
  }
  ...
  state.derived = visibleProjects.map(project => {
    ...
    return deriveProjectState(project, state.evidence, {
      inventoryPresent,
      currentChatCount
    });
  });

  // v0.8.1 legacy-inventory conservation fallback:
  // v0.7.2 stored authoritative API aggregate totals in CONTROL but omitted the per-project list.
  // If every inventoried conversation is already represented by current sources, the API says there
  // is exactly one more project than CONTROL has project-key mappings, and exactly one visible project
  // remains UNSYNCED/unmapped, that sole project must contain zero conversations. This is a strict
  // conservation proof, not a fuzzy title guess, and it repairs the old LRC Maker state without any
  // new ChatGPT crawl or synthetic conversation evidence.
  if (inventoryProjects.length === 0) {
    ...
    const representedConversationCount = [...currentChatCounts.values()].reduce((sum, count) => sum + Number(count || 0), 0);
    const mappedApiProjectCount = Object.keys(projectMappings).length;
    const mappedProjectIds = new Set(Object.values(projectMappings));
    const unmappedVisibleProjects = visibleProjects.filter(project => !mappedProjectIds.has(project.id));
    ...
    const conservationProvesSoleEmpty =
      ...
      representedConversationCount === apiConversationCount &&
      mappedApiProjectCount + 1 === apiProjectCount &&
      unmappedVisibleProjects.length === 1 &&
      unsyncedStates.length === 1 &&
      unmappedVisibleProjects[0].id === unsyncedStates[0].projectId;

    if (conservationProvesSoleEmpty) {
      const project = unmappedVisibleProjects[0];
      const index = state.derived.findIndex(item => item.projectId === project.id);
      if (index >= 0) {
        state.derived[index] = deriveProjectState(project, state.evidence, {
          inventoryPresent: true,
          currentChatCount: 0,
          inventoryProof: 'legacy-conservation'
        });
      }
    }
  }
