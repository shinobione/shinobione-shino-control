...
export function deriveAll(state) {
  ...
  // outlive projects, so mapping-count arithmetic is not trustworthy. The current 131 thread sources,
  // however, carry their real ChatGPT project keys. If they account for every API conversation and
  // span exactly API-project-count minus one distinct project keys, then exactly one API project is
  // empty. If CONTROL also has exactly one UNSYNCED project with no current ChatGPT thread, that is
  // the empty project. No fuzzy title matching and no new crawl are involved.
    ...
    if (conservationProvesSoleEmpty) {
      ...
      if (project && index >= 0) {
        state.derived[index] = deriveProjectState(project, state.evidence, {
          ...
          currentChatCount: 0,
          inventoryProof: 'legacy-conservation-current-project-keys'
        });
      }
    }
  }
