...
export function deriveProjectState(project, evidence, context = {}) {
  ...
  if (!usable.length) {
    // A remembered ChatGPT project route is enough to prove that the project is synced at the
    // project level. Some older/chat-only CONTROL projects do not carry kind=CHATGPT_PROJECT,
    // so requiring that flag incorrectly labelled a real zero-conversation project UNSYNCED.
    const apiConfirmedEmpty = context.chatgptMapped === true && Number(context.currentChatCount || 0) === 0;
    if (apiConfirmedEmpty) {
      return {
        projectId: project.id,
        status: 'EMPTY',
        summary: 'Projet ChatGPT correctement mappé, mais aucune conversation n’existe actuellement dans son inventaire API.',
        lastMovementAt: null,
...
export function deriveAll(state) {
  ...
  const mappedProjectIds = new Set(Object.values(state.settings.chatgptProjectMappings || {}));
  ...
  state.derived = state.projects
    ...
    .map(project => deriveProjectState(project, state.evidence, {
      chatgptMapped: mappedProjectIds.has(project.id),
      currentChatCount: currentChatCounts.get(project.id) || 0
    }));
