...
export function deriveAll(state, options = {}) {
  ...
  state.derived = visibleProjects.map(project => {
    const context = inventoryContextForProject(project, projectMappings, inventoryByKey, inventoryByName, currentChatCounts);
  ...
  // Keep the conservation proof as a compatibility bridge. It executes after cached states are
  // merged, so the one proven empty project can still be corrected without rederiving all others.
  if (inventoryProjects.length === 0) {
    ...
    if (conservationProvesSoleEmpty) {
      ...
      if (project && index >= 0 && stateItem.status !== 'EMPTY') {
        const projectEvidence = evidenceByProject.get(project.id) || [];
        state.derived[index] = {
          ...deriveProjectState(project, projectEvidence, {
        ...
        nextFingerprints[project.id] = projectDerivationFingerprint(project, projectEvidence, {
          inventoryPresent: true,
          currentChatCount: 0,
          inventoryProof: 'legacy-conservation-project-key-coverage'
        });
