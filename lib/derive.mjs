...
function statusSignal(e) {
  ...
  // Only hard hints are allowed to override textual derivation. The old ACTIVE hint was
  // generated from any stray "pass/fixed/works" token and flattened almost every project to ACTIVE.
...
function summaryFor(e) {
  ...
  // A chat transcript is evidence, not a trustworthy project-level status paragraph. Showing
  // arbitrary assistant prose here produced nonsense such as product names / pasted prompts.
...
export function deriveProjectState(project, evidence) {
  ...
  // Prefer authoritative non-chat state prose. If ChatGPT is all we have, report the latest
  // conversation title rather than pretending a transcript snippet is a project summary.
