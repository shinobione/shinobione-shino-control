...
function bind(){
  ...
  document.addEventListener('click',e=>{document.querySelectorAll('[data-status-picker].open').forEach(picker=>{if(!picker.contains(e.target)){picker.classList.remove('open');picker.querySelector('[data-status-trigger]')?.setAttribute('aria-expanded','false')}})},{once:true});
