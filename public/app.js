...
function ctAs(p,d){
  const ev=evidenceFor(p.id); const chat=sourcesFor(p.id).filter(s=>s.type==='chatgpt_thread'&&s.url).sort((a,b)=>new Date(b.lastObservedAt||0)-new Date(a.lastObservedAt||0))[0]; const pr=ev.find(e=>e.sourceType==='github_pr'&&e.url&&!/superseded/i.test(`${e.title} ${e.summary}`)); const repo=p.repo?`https://github.com/${p.repo}`:null;
  return {chat,pr,repo};
}
...
function radarView(s){
  let list=state.projects.filter(p=>{const d=projectState(p.id);return d&&(!query||`${p.name} ${p.universe} ${d?.summary||''}`.toLowerCase().includes(query.toLowerCase()))&&(statusFilter==='ALL'||d?.status===statusFilter)});
  ...
  <section class="stats"><div class="stat stat-active"><span>Active</span><b>${s.active}</b></div><div class="stat stat-test"><span>Needs test</span><b>${s.test}</b></div><div class="stat stat-blocked"><span>Blocked</span><b>${s.blocked}</b></div><div class="stat stat-stable"><span>Stable</span><b>${s.stable}</b></div><div class="stat stat-aging"><span>Aging sources</span><b>${s.stale}</b></div></section>
...
function projectCard(p){const d=projectState(p.id); if(!d)return ''; const src=sourcesFor(p.id); const e=evidenceFor(p.id)[0]; const c=ctAs(p,d);
  return `<article class="card ${statusClass(d.status)}" data-open-project="${p.id}"><div class="card-head"><div><h3>${esc(p.name)}</h3><div class="universe">${esc(p.universe)}</div></div><span class="pill ${esc(d.status)}">${esc(d.status)}</span></div>
    ...
    <div class="source-row">${src.map(s=>`<span class="chip ${sourceClass(s.type)}">${esc(sourceLabel(s))}</span>`).join('')}<span class="confidence">${esc(d.confidence)} confidence</span></div>
...
function sourcesView(){const gh=state.sources.filter(s=>s.type==='github_repo'), comps=state.sources.filter(s=>s.type==='github_component'), chats=state.sources.filter(s=>s.type==='chatgpt_thread');const mappedProjectKeys=Object.keys(state.settings?.chatgptProjectMappings||{}).length;return `<div class="topbar"><div class="titleblock"><h2>Sources / Sync</h2><p>GitHub + ChatGPT feed the radar. SHINO Sync is a CONTROL component, not a separate project.</p></div></div>
  <div class="panel panel-github" style="margin-bottom:12px"><h3>GitHub sync</h3><p class="note">Public repos sync without a token. For private repos like LaunchPAD, set <code>GITHUB_TOKEN</code> before starting the server, or paste a fine-grained token here for this one sync only. The server never writes that token to disk.</p><div class="config-row"><input id="ghToken" type="password" placeholder="Fine-grained GitHub token (optional, one sync only)"><button class="btn gold" id="syncBtn2">Sync configured repos</button></div></div>
  <div class="panel panel-chatgpt" style="margin-bottom:12px"><h3>ChatGPT auto-sync</h3><p class="note"><b>SHINO Sync lives inside SHINO // CONTROL.</b> ${mappedProjectKeys} ChatGPT project route${mappedProjectKeys===1?' is':'s are'} currently remembered. Once a project is mapped, every new conversation created inside that same ChatGPT project is auto-attached after its first messages render. Backfill is only needed for historical threads or newly discovered projects.</p></div>
